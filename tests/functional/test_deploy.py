...
sources = [('local', '{}/builds/dans-example'.format(juju_repository)),
           # ('jujucharms', 'cs:...'),
           ]
...
async def app(model, series, source):
    app_name = 'dans-example-{}-{}'.format(series, source[0])
    return await model._wait_for_new('application', app_name)


async def test_dansexample_deploy(model, series, source, request):
    # Starts a deploy for each series
    # Using subprocess b/c libjuju fails with JAAS
    # https://github.com/juju/python-libjuju/issues/221
    application_name = 'dans-example-{}-{}'.format(series, source[0])
...
# Tests
async def test_dansexample_status(model, app):
